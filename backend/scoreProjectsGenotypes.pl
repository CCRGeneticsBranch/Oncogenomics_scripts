...
Options:
  ...
  -p  <string>  project IDs (comma saperated)
...
close(FH);
my $sql = "select * from genotyping g where exists(select * from project_samples s1 where s1.project_id in ($project_id) and g.sample1=s1.sample_id)  and exists(select * from project_samples s2 where s2.project_id in ($project_id) and g.sample2=s2.sample_id)";
...
$sth_gt_db->finish;
print("done\n");
$sql = "select distinct s.path,s.patient_id,s.sample_id,s.sample_name,s.case_id from project_samples p, processed_sample_cases s where project_id in ($project_id) and p.sample_id=s.sample_id order by sample_id";
