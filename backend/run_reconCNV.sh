...
if [ ! -f ${bn}.html ];then
	...
	python ${reconCNV_home}/reconCNV.py -r ${cleaned_cnr} -g ${cleaned_cnr} -x ${reconCNV_home}/data/hg19_genome_length_chr.txt -a ${reconCNV_home}/data/hg19_COSMIC_genes_model_chr.txt -s $cns -c ${reconCNV_home}/config.json -o ${bn}.html -d $dn
	chrs=()
	...
	for chr in "${chrs[@]}";do
		...
		python ${reconCNV_home}/reconCNV.py -r ${chr_cnr} -g ${chr_cnr} -x ${chr_len} -a ${reconCNV_home}/data/annotation/hg19/${chr}.annotation.txt -s ${chr_cns} -c ${reconCNV_home}/config.json -o ${bn}.${chr}.html -d $dn
