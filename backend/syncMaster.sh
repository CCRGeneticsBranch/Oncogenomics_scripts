...
if [ -z $projects ];then
	if [[ $no_change = "N" ]];then
		...
		perl $script_home_production/syncMaster.pl -u -i $file_list -m $flag_list -g $project_group_list
		 if [[ "$AWS" != "true" ]];then
			...
			echo `date +"%Y-%m-%d %H:%M:%S"` "$script_home_dev/syncMaster.pl -u -i $file_list -m $flag_list -g $project_group_list"		
			perl $script_home_dev/syncMaster.pl -u -i $file_list -m $flag_list -g $project_group_list
		fi
